...
#include <string>
#include <z3.h>
#include "alivesmt/ctx.h"
#include "alivesmt/exprs.h"
#include "alivesmt/smt.h"
...
static Z3_sort mkBVSort(unsigned bits) {
  assert(bits > 0);
  return Z3_mk_bv_sort(alivesmt::ctx(), bits);
}

static Z3_ast mkVar(const char *name, Z3_sort sort) {
  return Z3_mk_const(alivesmt::ctx(), Z3_mk_string_symbol(alivesmt::ctx(), name), sort);
}

static expr simplify_const(expr &&e) { return e.simplifyNoTimeout(); }

template<typename... Exprs>
static expr simplify_const(expr &&e, const expr &input, const Exprs &...inputs) {
  if (input.isConst())
...
namespace alivesmt {
  ...
  Z3_sort expr::sort() const { return Z3_get_sort(ctx(), ast()); }
  ...
  Z3_ast expr::mkTrue() { return Z3_mk_true(ctx()); }

  Z3_ast expr::mkFalse() { return Z3_mk_false(ctx()); }

  expr expr::mkUInt(uint64_t n, Z3_sort sort) { return Z3_mk_unsigned_int64(ctx(), n, sort); }

  expr expr::mkUInt(uint64_t n, unsigned bits) { return bits ? mkUInt(n, mkBVSort(bits)) : expr(); }
  ...
  expr expr::mkInt(int64_t n, Z3_sort sort) { return Z3_mk_int64(ctx(), n, sort); }

  expr expr::mkInt(int64_t n, unsigned bits) { return bits ? mkInt(n, mkBVSort(bits)) : expr(); }
  ...
  static Z3_sort mk_bfloat_sort() { return Z3_mk_fpa_sort(ctx(), 8, 8); }

  expr expr::mkBFloat(float n) { return Z3_mk_fpa_numeral_float(ctx(), n, mk_bfloat_sort()); }
  ...
  expr expr::mkConst(Z3_decl decl) { return Z3_mk_app(ctx(), decl, 0, {}); }
  ...
  expr expr::mkQVar(unsigned n, unsigned bits) { return Z3_mk_bound(ctx(), n, mkBVSort(bits)); }
  ...
  expr expr::mkBoolVar(const char *name) { return ::mkVar(name, Z3_mk_bool_sort(ctx())); }

  expr expr::mkHalfVar(const char *name) { return ::mkVar(name, Z3_mk_fpa_sort_half(ctx())); }

  expr expr::mkBFloatVar(const char *name) { return ::mkVar(name, mk_bfloat_sort()); }

  expr expr::mkFloatVar(const char *name) { return ::mkVar(name, Z3_mk_fpa_sort_single(ctx())); }

  expr expr::mkDoubleVar(const char *name) { return ::mkVar(name, Z3_mk_fpa_sort_double(ctx())); }

  expr expr::mkQuadVar(const char *name) { return ::mkVar(name, Z3_mk_fpa_sort_quadruple(ctx())); }
  ...
  expr expr::some(const expr &type) { return type.isBool() ? expr(false) : mkNumber("3", type); }
  ...
  expr expr::IntUMax(unsigned bits) { return mkInt(-1, bits); }
  ...
  bool expr::isConst() const {
    C();
    return Z3_is_numeral_ast(ctx(), ast()) || Z3_get_bool_value(ctx(), ast()) != Z3_L_UNDEF;
  }
  ...
  bool expr::isSMin() const { return eq(IntSMin(bits())); }

  bool expr::isSMax() const { return eq(IntSMax(bits())); }

  expr expr::isNegative() const { return sign() == 1; }
  ...
  bool expr::isEq(expr &lhs, expr &rhs) const { return isBinOp(lhs, rhs, Z3_OP_EQ); }

  bool expr::isSLE(expr &lhs, expr &rhs) const { return isBinOp(lhs, rhs, Z3_OP_SLEQ); }

  bool expr::isULE(expr &lhs, expr &rhs) const { return isBinOp(lhs, rhs, Z3_OP_ULEQ); }
  ...
  bool expr::isSignExt(expr &val) const { return isUnOp(val, Z3_OP_SIGN_EXT); }

  bool expr::isAShr(expr &a, expr &b) const { return isBinOp(a, b, Z3_OP_BASHR); }

  bool expr::isAnd(expr &a, expr &b) const { return isBinOp(a, b, Z3_OP_AND); }

  bool expr::isNot(expr &neg) const { return isUnOp(neg, Z3_OP_NOT); }

  bool expr::isAdd(expr &a, expr &b) const { return isBinOp(a, b, Z3_OP_BADD); }

  bool expr::isURem(expr &a, expr &b) const { return isBinOp(a, b, Z3_OP_BUREM); }
  ...
  bool expr::isConstArray(expr &val) const { return isUnOp(val, Z3_OP_CONST_ARRAY); }

  bool expr::isStore(expr &array, expr &idx, expr &val) const {
    return isTernaryOp(array, idx, val, Z3_OP_STORE);
  }

  bool expr::isLoad(expr &array, expr &idx) const { return isBinOp(array, idx, Z3_OP_SELECT); }
  ...
  bool expr::isFPNeg(expr &val) const {
    if (isBV()) {
      ...
      return isConcat(sign, rest) && sign.isBinOp(a, b, Z3_OP_BXOR) &&
             (check_not(a, b) || check_not(b, a)) && rest.isExtract(val2, high2, low) && low == 0 &&
             high2 == high - 1 && val.eq(val2);
    }
    return isUnOp(val, Z3_OP_FPA_NEG);
  }

  bool expr::isFAbs(expr &val) const { return isUnOp(val, Z3_OP_FPA_ABS); }
  ...
  bool expr::isIsFPZero() const {
    if (isBV()) {
      ...
      expr lhs, rhs, v;
      unsigned high, low;
      return isEq(lhs, rhs) && lhs.isExtract(v, high, low) && high == bits() - 2 && low == 0 &&
             rhs.isZero();
    }
  ...
  bool expr::isfloat2BV(expr &fp) const { return isUnOp(fp, Z3_OP_FPA_TO_IEEE_BV); }
  ...
  expr expr::binop_commutative(
      const expr &rhs, Z3_ast (*op)(Z3_context, Z3_ast, Z3_ast),
      expr (expr::*expr_op)(const expr &) const, bool (expr::*identity)() const,
      bool (expr::*absorvent)() const, int z3_app
  ) const {
    ...
    if (z3_app) {
      if (isConst()) {
        ...
        return rhs.binop_commutative(*this, op, expr_op, identity, absorvent, z3_app);
    }

    return binop_commutative(rhs, op);
  }

  expr expr::binop_commutative(const expr &rhs, Z3_ast (*op)(Z3_context, Z3_ast, Z3_ast)) const {
    assert(!isValid() || !rhs.isValid() || sort() == rhs.sort());
    auto cmp = *this < rhs;
    return (cmp ? *this : rhs).binop_fold(cmp ? rhs : *this, op);
  }

  expr expr::binop_fold(const expr &rhs, Z3_ast (*op)(Z3_context, Z3_ast, Z3_ast)) const {
    C(rhs);
...
#define binopc(op, exprop, z3app, identity, absorvent)                                             \
  binop_commutative(rhs, op, &expr::exprop, &expr::identity, &expr::absorvent, z3app)
  ...
  expr expr::operator+(const expr &rhs) const {
    ...
    if (rhs.isConst() && isConcat(a, b) && b.isConst()) {
      ...
      if (rhs2.zextOrTrunc(rhs.bits()).eq(rhs) && b.add_no_uoverflow(rhs2).isTrue())
  ...
  expr expr::sadd_sat(const expr &rhs) const {
    ...
    auto min = IntSMin(bw);
    auto max = IntSMax(bw);
    return mkIf(add_ext.sle(min.sext(1)), min, mkIf(add_ext.sge(max.sext(1)), max, *this + rhs));
  }
  ...
  expr expr::ssub_sat(const expr &rhs) const {
    ...
    auto min = IntSMin(bw);
    auto max = IntSMax(bw);
    return mkIf(sub_ext.sle(min.sext(1)), min, mkIf(sub_ext.sge(max.sext(1)), max, *this - rhs));
  }
  ...
  expr expr::sshl_sat(const expr &rhs) const {
    C();
    return mkIf(
        shl_no_soverflow(rhs), *this << rhs, mkIf(isNegative(), IntSMin(bits()), IntSMax(bits()))
    );
  }
  ...
  expr expr::sub_no_uoverflow(const expr &rhs) const { return (zext(1) - rhs.zext(1)).sign() == 0; }
  ...
  expr expr::sdiv_exact(const expr &rhs) const { return sdiv(rhs) * rhs == *this; }

  expr expr::udiv_exact(const expr &rhs) const { return udiv(rhs) * rhs == *this; }
  ...
  expr expr::smul_fix_sat(const expr &a, const expr &b, const expr &c) {
    ...
    auto width = a.bits();
    return mkIf(
        smul_fix_no_soverflow(a, b, c), smul_fix(a, b, c),
  ...
  expr expr::umul_fix_sat(const expr &a, const expr &b, const expr &c) {
    auto width = a.bits();
    return mkIf(umul_fix_no_uoverflow(a, b, c), umul_fix(a, b, c), IntUMax(width));
  }

  expr expr::shl_no_soverflow(const expr &rhs) const { return (*this << rhs).ashr(rhs) == *this; }

  expr expr::shl_no_uoverflow(const expr &rhs) const { return (*this << rhs).lshr(rhs) == *this; }

  expr expr::ashr_exact(const expr &rhs) const { return (ashr(rhs) << rhs) == *this; }

  expr expr::lshr_exact(const expr &rhs) const { return (lshr(rhs) << rhs) == *this; }

  static expr log2_rec(const expr &e, unsigned idx, unsigned bw) {
    if (idx == 0)
      return expr::mkUInt(0, bw);

    return expr::mkIf(e.extract(idx, idx) == 1, expr::mkUInt(idx, bw), log2_rec(e, idx - 1, bw));
  }

  expr expr::isPowerOf2() const { return *this != 0 && (*this & (*this - mkUInt(1, *this))) == 0; }
  ...
  expr expr::umin(const expr &rhs) const { return mkIf(ule(rhs), *this, rhs); }

  expr expr::umax(const expr &rhs) const { return mkIf(uge(rhs), *this, rhs); }

  expr expr::smin(const expr &rhs) const { return mkIf(sle(rhs), *this, rhs); }

  expr expr::smax(const expr &rhs) const { return mkIf(sge(rhs), *this, rhs); }
...
#define fold_fp_neg(fn)                                                                            \
  ...
  expr expr::isFPNegative() const { return unop_fold(Z3_mk_fpa_is_negative); }

  expr expr::isFPNegZero() const { return isFPZero() && isFPNegative(); }

  expr expr::isFPNormal() const { return unop_fold(Z3_mk_fpa_is_normal); }

  expr expr::isFPSubNormal() const { return unop_fold(Z3_mk_fpa_is_subnormal); }

  expr expr::rne() { return Z3_mk_fpa_rne(ctx()); }

  expr expr::rna() { return Z3_mk_fpa_rna(ctx()); }

  expr expr::rtp() { return Z3_mk_fpa_rtp(ctx()); }

  expr expr::rtn() { return Z3_mk_fpa_rtn(ctx()); }

  expr expr::rtz() { return Z3_mk_fpa_rtz(ctx()); }
  ...
  std::pair<expr, expr> expr::frexp() const {
    ...
    exponent = expr::mkIf(
        ...
        expr::mkIf(
            subnormal, expr::mkInt(1 - bias, 32) - shift.sextOrTrunc(32),
    ...
    mantissa = expr::mkIf(isFPZero(), *this, expr::mkIf(sign == 0, mantissa, mantissa.fneg()));
  ...
  expr expr::ceil() const { return round(rtp()); }

  expr expr::floor() const { return round(rtn()); }
  ...
  expr expr::foeq(const expr &rhs) const { return binop_commutative(rhs, Z3_mk_fpa_eq); }

  expr expr::fogt(const expr &rhs) const { return binop_fold(rhs, Z3_mk_fpa_gt); }

  expr expr::foge(const expr &rhs) const { return binop_fold(rhs, Z3_mk_fpa_geq); }

  expr expr::folt(const expr &rhs) const { return binop_fold(rhs, Z3_mk_fpa_lt); }

  expr expr::fole(const expr &rhs) const { return binop_fold(rhs, Z3_mk_fpa_leq); }

  expr expr::fone(const expr &rhs) const {
    return ford(rhs) && !binop_commutative(rhs, Z3_mk_fpa_eq);
  }

  expr expr::ford(const expr &rhs) const { return !isNaN() && !rhs.isNaN(); }

  expr expr::fueq(const expr &rhs) const {
    return funo(rhs) || binop_commutative(rhs, Z3_mk_fpa_eq);
  }

  expr expr::fugt(const expr &rhs) const { return funo(rhs) || binop_fold(rhs, Z3_mk_fpa_gt); }

  expr expr::fuge(const expr &rhs) const { return funo(rhs) || binop_fold(rhs, Z3_mk_fpa_geq); }

  expr expr::fult(const expr &rhs) const { return funo(rhs) || binop_fold(rhs, Z3_mk_fpa_lt); }

  expr expr::fule(const expr &rhs) const { return funo(rhs) || binop_fold(rhs, Z3_mk_fpa_leq); }

  expr expr::fune(const expr &rhs) const {
    return funo(rhs) || !binop_commutative(rhs, Z3_mk_fpa_eq);
  }

  expr expr::funo(const expr &rhs) const { return isNaN() || rhs.isNaN(); }
  ...
  expr expr::operator~() const { return unop_fold(Z3_mk_bvnot); }
  ...
  expr expr::cmp_eq(const expr &rhs, bool simplify) const {
    ...
    // (= (= a bit) (= b bit)) -> (= a b)
    {
      ...
      if (isEq(lhs_a, lhs_b) && lhs_a.isBV() && lhs_a.bits() == 1 && rhs.isEq(rhs_a, rhs_b) &&
          lhs_a.eq(rhs_a))
        return lhs_b == rhs_b;
    }
    ...
    // (= (+ a c1) (+ a c2)) -> false
    {
      ...
      if (isBasePlusOffset(lhs_base, lhs_offset) && rhs.isBasePlusOffset(rhs_base, rhs_offset) &&
          lhs_base.eq(rhs_base) && lhs_offset != rhs_offset)
        return false;
    }
        ...
        for (unsigned i = 0; i < num_args; ++i) {
          ...
          // bits in an attempt to prove the comparison false
            ...
            if (l_idx >= r_idx && l_idx < (r_idx + r_bits)) {
              ...
              eqs.add(
                  lhs.extract(l_bits - 1, l_bits - overlap) ==
              ...
              eqs.add(
                  lhs.extract(l_off + overlap - 1, l_off) ==
...

    {
      ...
      if (isIf(c, t, e)) {
        ...
        expr c2, t2, e2;
        if (rhs.isIf(c2, t2, e2) && t.eq(t2) && e.eq(e2) && !t.eq(e) && t.isConst() && e.isConst())
          return c == c2;
  ...
  expr expr::operator==(const expr &rhs) const { return cmp_eq(rhs, true); }
  ...
  void expr::operator&=(const expr &rhs) { *this = *this && rhs; }
  ...
  expr expr::uge(const expr &rhs) const { return rhs.ule(*this); }
  ...
  expr expr::sge(const expr &rhs) const { return rhs.sle(*this); }

  expr expr::sgt(const expr &rhs) const { return !sle(rhs); }

  expr expr::ule(uint64_t rhs) const { return ule(mkUInt(rhs, *this)); }
  ...
  expr expr::ult(uint64_t rhs) const { return ult(mkUInt(rhs, *this)); }

  expr expr::uge(uint64_t rhs) const { return uge(mkUInt(rhs, *this)); }

  expr expr::ugt(uint64_t rhs) const { return ugt(mkUInt(rhs, *this)); }

  expr expr::sle(int64_t rhs) const { return sle(mkInt(rhs, *this)); }

  expr expr::sge(int64_t rhs) const { return sge(mkInt(rhs, *this)); }

  expr expr::operator==(uint64_t rhs) const { return *this == mkUInt(rhs, *this); }
  ...
  expr expr::trunc(unsigned tobw) const { return extract(tobw - 1, 0); }
  ...
  expr expr::concat(const expr &rhs) const {
    ...
    if (isExtract(a, h, l)) {
      if (rhs.isExtract(b, h2, l2) && l == h2 + 1) {
        ...
        expr aa, ab;
        if (l2 == 0 && a.isConcat(aa, ab) && ab.eq(b) && (h - aa.bits()) == b.bits())
          return aa.concat(b);
      }
    ...
    if (isConcat(a, b) && b.isExtract(c, h, l) && rhs.isExtract(d, h2, l2) && l == h2 + 1 &&
        c.eq(d))
  ...
  expr expr::concat_zeros(unsigned bits) const { return bits ? concat(mkUInt(0, bits)) : *this; }
    ...
    }
    {
      auto simpl_bitwise = [&](int type, bool (expr::*absorvent)() const) {
        if (auto app = isAppOf(type)) {
          ...
          for (unsigned i = 0; i < num_args; ++i) {
            expr arg = expr(Z3_get_app_arg(ctx(), app, i)).extract(high, low, depth);
  ...
  expr expr::float2Real() const { return unop_fold(Z3_mk_fpa_to_real); }
  ...
  expr expr::float2Float(const expr &type, const expr &rm) const {
    C(type, rm);
    return simplify_const(Z3_mk_fpa_to_fp_float(ctx(), rm(), ast(), type.sort()), *this);
  }
  ...
  expr expr::sint2fp(const expr &type, const expr &rm) const {
    C(type, rm);
    return simplify_const(Z3_mk_fpa_to_fp_signed(ctx(), rm(), ast(), type.sort()), *this);
  }

  expr expr::uint2fp(const expr &type, const expr &rm) const {
    C(type, rm);
    return simplify_const(Z3_mk_fpa_to_fp_unsigned(ctx(), rm(), ast(), type.sort()), *this);
  }
  ...
  expr expr::mkUF(const char *name, const vector<expr> &args, const expr &range) {
    ...
    auto decl = Z3_mk_func_decl(
        ctx(), Z3_mk_string_symbol(ctx(), name), num_args, z3_sorts.data(), range.sort()
    );
  ...
  expr expr::mkIf(const expr &cond, const expr &then, const expr &els) {
    ...
    // (ite (= x 1) 1 0) -> x
    // (ite (= x 1) 0 1) -> (not x)
    if (then.isBV() && then.bits() == 1 && cond.isEq(lhs, rhs) && lhs.isBV() && lhs.bits() == 1) {
      if (then.isOne() && els.isZero()) {
        if (lhs.isOne())
          return rhs;
        if (rhs.isOne())
  ...
  expr expr::mkForAll(const set<expr> &vars, expr &&val) {
    ...
    return Z3_mk_forall_const(ctx(), 0, vars.size(), vars_ast.get(), 0, nullptr, val());
  }

  expr expr::mkForAll(unsigned num_vars, const expr *vars, const char **names, expr &&val) {
  ...
  expr expr::foldTopLevel() const {
    expr cond, then, els;
    if (isIf(cond, then, els))
      return expr::mkIf(cond.foldTopLevel(), then.foldTopLevel(), els.foldTopLevel());
  ...
  set<expr> expr::vars() const { return vars({this}); }
  ...
  void expr::printUnsigned(ostream &os) const { os << numeral_string(); }
