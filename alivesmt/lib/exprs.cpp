...
namespace alivesmt {
  ...
  void AndExpr::add(const AndExpr &other) { exprs.insert(other.exprs.begin(), other.exprs.end()); }
  ...
  void AndExpr::reset() { exprs.clear(); }

  bool AndExpr::contains(const expr &e) const { return exprs.count(e); }

  expr AndExpr::operator()() const { return expr::mk_and(exprs); }

  AndExpr::operator bool() const { return !exprs.count(false); }

  ostream &operator<<(ostream &os, const AndExpr &e) { return os << e(); }
  ...
  void OrExpr::add(const OrExpr &other) { exprs.insert(other.exprs.begin(), other.exprs.end()); }

  expr OrExpr::operator()() const { return expr::mk_or(exprs); }

  ostream &operator<<(ostream &os, const OrExpr &e) { return os << e(); }
  ...
  static expr simplify(const expr &e, const expr &cond, bool negate) {
    ...
    if (e.isAdd(a, b)) {
      auto test = [&](const expr &a, const expr &b) -> expr {
        ...
        unsigned high, low;
        if (b.isExtract(extract, high, low) && extract.isIf(ifcond, t, e) && ifcond.eq(cond)) {
          return a + (negate ? e : t).extract(high, low);
        }
        return {};
      };
      if (auto r = test(a, b); r.isValid())
        return r;
      if (auto r = test(b, a); r.isValid())
        return r;
    }
    return e;
  }
  ...
  DisjointExpr<expr>::DisjointExpr(const expr &e, unsigned depth_limit) {
    ...
    do {
      ...
      if ((worklist.size() + vals.size()) >= 32 || hit_half_memory_limit()) {
        ...
        for (auto &[lhs_v, lhs_domain]: lhs) {
          if (auto rhs_val = rhs.lookup(lhs_domain)) {
            ...
            for (auto &[rhs_v, rhs_domain]: rhs) {
              add(lhs_v.concat(simplify(rhs_v, from, negate)), c && lhs_domain && rhs_domain);
            }
          }
        }
  ...
  template<>
  AndExpr DisjointExpr<AndExpr>::factor() const {
    assert(!vals.empty());
    if (vals.size() == 1)
  ...
  void FunctionExpr::add(const FunctionExpr &other) { fn.insert(other.fn.begin(), other.fn.end()); }
  ...
  FunctionExpr
  FunctionExpr::mkIf(const expr &cond, const FunctionExpr &then, const FunctionExpr &els) {
    FunctionExpr ret;
    auto I = els.begin();
    for (auto &[k, v]: then) {
      assert(I != els.end());
