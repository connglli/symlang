...
namespace {
  ...
  struct Goal {
    ...
    Goal(Goal &&g) noexcept { swap(goal, g.goal); }

    void operator=(Goal &&g) noexcept { swap(goal, g.goal); }
    ...
    bool isDecided() const {
      return Z3_goal_is_decided_sat(ctx(), goal) || Z3_goal_is_decided_unsat(ctx(), goal);
    }

    const char *toString() const { return Z3_goal_to_string(ctx(), goal); }
  };
  ...
  struct TacticResult {
    ...
    TacticResult(TacticResult &&other) noexcept { swap(r, other.r); }
    ...
    void operator=(TacticResult &&other) { swap(r, other.r); }

    Goal toGoal() const {
      Goal goal;
      for (unsigned i = 0, e = Z3_apply_result_get_num_subgoals(ctx(), r); i != e; ++i) {
  ...
  struct Probe {
    ...
    bool check(const Goal &goal) const { return Z3_probe_apply(ctx(), p, goal.goal) != 0.0; }
  };
  ...
  struct Tactic {
    ...
    Tactic(Tactic &&other) noexcept { swap(t, other.t); }
    ...
    virtual TacticResult exec([[maybe_unused]] const Goal &goal) const {
      UNREACHABLE();
      return {};
    };
  };
  ...
  struct NamedTactic final : public Tactic {
    ...
    NamedTactic(const char *name) : Tactic(Z3_mk_tactic(ctx(), name)) { this->name = name; }

    TacticResult exec(const Goal &goal) const override {
      dbg() << "\nApplying " << name << endl;
      Tactic to(Z3_tactic_or_else(
          ctx(), Tactic(Z3_tactic_try_for(ctx(), t, 10000)).t, Tactic(Z3_tactic_skip(ctx())).t
      ));
  ...
  struct IfTactic final : public Tactic {
    ...
    IfTactic(Probe &&p, unique_ptr<Tactic> &&then, unique_ptr<Tactic> &&els) :
        Tactic(Z3_tactic_cond(ctx(), p.p, then->t, els->t)), p(std::move(p)), then(std::move(then)),
        els(std::move(els)) {}
  ...
  class AndTactic final : public Tactic {
    ...
    void append(Z3_tactic t) { set(this->t ? Z3_tactic_and_then(ctx(), this->t, t) : t); }
  ...
  public:
    ...
    void appendIf(const char *probe, T1 &&then, T2 &&els) {
      IfTactic t(probe, make_unique<T1>(std::move(then)), make_unique<T2>(std::move(els)));
      append(t.t);
      if (tactic_verbose)
  ...
  public:
    ...
    Z3_solver getSolver() const { return Z3_mk_solver_from_tactic(ctx(), tactics.t); }

    friend class alivesmt::Solver;
  };
} // namespace
...
namespace alivesmt {
  ...
  Model::Model(Z3_model m) : m(m) { Z3_model_inc_ref(ctx(), m); }
  ...
  Model::iterator Model::end() const { return {nullptr, Z3_model_get_num_consts(ctx(), m)}; }
  ...
  ostream &operator<<(ostream &os, const Model &m) { return os << Z3_model_to_string(ctx(), m.m); }

  FnModel::FnModel(Z3_func_interp f, const string &fn_name) : f(f), fn_name(fn_name) {
  ...
  pair<expr, expr> FnModel::iterator::operator*(void) const {
    ...
    vector<expr> args;
    for (unsigned i = 0, e = Z3_func_entry_get_num_args(ctx(), entry); i != e; ++i) {
  ...
  void solver_print_queries(bool yes) { print_queries = yes; }

  void solver_tactic_verbose(bool yes) { tactic_verbose = yes; }

  Solver::Solver(bool simple) {
    s = simple ? Z3_mk_simple_solver(ctx()) : tactic->getSolver();
...
; More info in "Alive2: Bounded Translation Validation for LLVM", PLDI'21.)";
      expr fml = assertions();
      if (!fml.isTrue()) {
        auto str = Z3_benchmark_to_smtlib_string(
            ctx(), banner, nullptr, nullptr, nullptr, 0, nullptr, fml()
        );
  ...
  void solver_print_stats(ostream &os) {
    ...
    float trivial_pc = num_queries == 0 ? 0 : (num_trivial * 100.0) / (num_trivial + num_queries);
  ...
  EnableSMTQueriesTMP::EnableSMTQueriesTMP() : old(config::skip_smt) { config::skip_smt = false; }

  EnableSMTQueriesTMP::~EnableSMTQueriesTMP() { config::skip_smt = old; }

  void solver_init() {
    tactic.emplace(
        {"simplify", "propagate-values", "simplify", "elim-uncnstr", "qe-light", "simplify",
         "elim-uncnstr", "reduce-args", "qe-light", "simplify", "smt"}
    );
#if 0
  ...
  void solver_destroy() { tactic.reset(); }
