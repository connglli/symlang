...
namespace alivesmt {
  ...
  public:
    ...
    void add(V &&val, D &&domain) {
      ...
      auto [I, inserted] = vals.try_emplace(std::forward<V>(val), std::forward<D>(domain));
      if (!inserted)
    ...
    std::optional<T> mk(std::optional<T> ret) && {
      for (auto &[val0, domain]: vals) {
        ...
        ret = ret ? T::mkIf(domain, std::move(val), *std::move(ret)) : std::move(val);
      }
      if (ret)
        return ret;
  ...
  public:
    ...
    static FunctionExpr mkIf(const expr &cond, const FunctionExpr &then, const FunctionExpr &els);
...
static inline alivesmt::expr merge(std::pair<alivesmt::AndExpr, alivesmt::expr> e) {
