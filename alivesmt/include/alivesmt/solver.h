...
namespace alivesmt {
  ...
  public:
    Model(Model &&other) noexcept { std::swap(other.m, m); }
  ...
  public:
    ...
    Result check(const char *query_name, bool dont_skip = false) const;

    friend class SolverPush;
  };

  Result check_expr(const expr &e, const char *query_name, bool dont_skip = false);
