...
namespace alivesmt {
  ...
  class expr {
    ...
    expr binop_commutative(
        const expr &rhs, Z3_ast (*op)(Z3_context, Z3_ast, Z3_ast),
        expr (expr::*expr_op)(const expr &) const, bool (expr::*identity)() const,
        bool (expr::*absorvent)() const, int z3_app = 0
    ) const;
    expr binop_commutative(const expr &rhs, Z3_ast (*op)(Z3_context, Z3_ast, Z3_ast)) const;

    expr unop_fold(Z3_ast (*op)(Z3_context, Z3_ast)) const;
    expr binop_fold(const expr &rhs, Z3_ast (*op)(Z3_context, Z3_ast, Z3_ast)) const;
  ...
  public:
    ...
    expr(expr &&other) noexcept { std::swap(ptr, other.ptr); }
    ...
    // of the desired type
    static expr mkUF(const char *name, const std::vector<expr> &args, const expr &range);

    static expr mkUF(const std::string &name, const std::vector<expr> &args, const expr &range) {
    ...
    static expr mkForAll(unsigned num_vars, const expr *vars, const char **names, expr &&val);
...
#define mkIf_fold(c, a, b)                                                                         \
  mkIf_fold_fn<std::remove_cvref_t<decltype(a)>>(                                                  \
      c, [&]() -> decltype(auto) { return a; }, [&]() -> decltype(auto) { return b; }              \
  )
