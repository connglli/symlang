...
namespace bitwuzla {
  ...
  private:
    ...
    static std::vector<bzla::Node> term_vector_to_nodes(const std::vector<Term> &terms);
    /** Convert vector of internal nodes to terms. */
    static std::vector<bitwuzla::Term> node_vector_to_terms(const std::vector<bzla::Node> &nodes);
   ...
   */
  template<>
  std::tuple<std::string, std::string, std::string> Term::value(uint8_t base) const;
#endif
  ...
  private:
    ...
    static std::vector<bzla::Type> sort_vector_to_types(const std::vector<Sort> &sorts);
  ...
  public:
     ...
     */
    Sort mk_uninterpreted_sort(const std::optional<std::string> &symbol = std::nullopt);
     ...
     */
    Term
    mk_fp_value(const Sort &sort, const Term &rm, const std::string &num, const std::string &den);
     ...
     */
    Term
    mk_term(Kind kind, const std::vector<Term> &args, const std::vector<uint64_t> &indices = {});
