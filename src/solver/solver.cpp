        ...
        [&](auto &&arg) -> SymbolicValue {
          ...
          if constexpr (std::is_same_v<T, OpAtom>) {
            ...
            if (nm == "@clz" || nm == "@ctz") {
              ...
              // We build from the back: start with default 0 (never reached
              // because x != 0), wrap with ITE for each lower-index match.
              smt::Term defaultT = solver.make_bv_value_int64(bvN, N);
              smt::Term result = defaultT;
              if (nm == "@clz") {
                // Iterate from i = N-1 down to 0; ITE(bit(N-1-i) == 1, i, result).
                for (int i = (int) N - 1; i >= 0; --i) {
                  int bitPos = (int) N - 1 - i;
                  auto bit = solver.make_term(
                      smt::Kind::BV_EXTRACT, {x}, {(uint32_t) bitPos, (uint32_t) bitPos}
                  );
                ...
                for (int i = (int) N - 1; i >= 0; --i) {
                  auto bit =
                      solver.make_term(smt::Kind::BV_EXTRACT, {x}, {(uint32_t) i, (uint32_t) i});
                  auto one1 = solver.make_bv_value_int64(solver.make_bv_sort(1), 1);
                  auto cond = solver.make_term(smt::Kind::EQUAL, {bit, one1});
                  auto iT = solver.make_bv_value_int64(bvN, i);
                  result = solver.make_term(smt::Kind::ITE, {cond, iT, result});
