...
- **`.siro` compiled object format** — deferred to at least v3.0.0. Source-level linking via `-I` is sufficient for v0.2.2. A serialized, pre-checked object format would speed up repeated builds but adds a persistence layer, versioning, and ABI concerns not yet needed.
...
- **Contract memory havoc** — planned refinement of §9.6.2 step 4. The current solver evaluates `pre` and `post` clauses correctly but does not yet havoc every cell of each pointer parameter's provenance object. Post clauses that talk about post-call pointee memory are sound only when the caller has already constrained that memory; an explicit havoc pass over the provenance object will close this gap in a follow-up.
- **Variadic functions**. All functions have a fixed, statically-known parameter list. Variadic calls complicate type-checking and SMT encoding without clear benefit for synthesis.
...
- **Tail-call semantics**. Calls are ordinary calls; tail-call elimination is a lowering concern, not a semantic one.
- **SSA / phi nodes**.
